...
static Token *function(Token *tok, Type *basety, VarAttr *attr) {
  ...
  // A buffer for a struct/union return value is passed as the hidden first
  // parameter.
  //
  // - x86-64 SysV: only for aggregates > 16 bytes.
  // - i386 cdecl: use sret for all aggregates (simplifies codegen and matches
  //   common ABI behavior).
  Type *rty = ty->return_ty;
  if (rty->kind == TY_STRUCT || rty->kind == TY_UNION) {
    if (target_arch == ARCH_I386 || rty->size > 16)
