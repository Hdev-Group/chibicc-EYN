...
void init_macros(void) {
  ...
  // Note: chibicc is a hosted compiler, but these macros describe the *target*.
  ...
  define_macro("__SIZEOF_LONG__", format("%d", target_long_size));
  define_macro("__SIZEOF_POINTER__", format("%d", target_ptr_size));
  define_macro("__SIZEOF_PTRDIFF_T__", format("%d", target_ptrdiff_t_size));
  define_macro("__SIZEOF_SHORT__", "2");
  define_macro("__SIZEOF_SIZE_T__", format("%d", target_size_t_size));
  // For ILP32 targets, 'unsigned long' is still 32-bit and widely accepted.
  define_macro("__SIZE_TYPE__", "unsigned long");
  define_macro("__STDC_HOSTED__", (target_os == OS_EYNOS) ? "0" : "1");
